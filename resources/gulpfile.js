...
gulp.task('typedefs', function() {
  ...
  var writePath = path.join(__dirname, '../pages/generated/sequins.d.json');
  var contents = JSON.stringify(genTypeDefData(typeDefPath, fileContents));
...
function gulpJS(subDir) {
  ...
  var sequinsGlobalModulePath = path.relative(
    path.resolve(SRC_DIR + subDir),
    path.resolve('./sequins-global.js'),
  );
  return function() {
    return (
      browserify({
        ...
        .require(sequinsGlobalModulePath, { expose: 'sequins' })
        // Helpful when developing with no wifi
        // .require('react', { expose: 'react' })
        // .require('sequins', { expose: 'sequins' })
...
gulp.task('sequins-copy', function() {
  return gulp
    .src(SRC_DIR + '../../dist/sequins.js')
...
gulp.task('build', function(done) {
  sequence(
    ['typedefs'],
    ['readme'],
    ['js', 'js-docs', 'less', 'less-docs', 'sequins-copy', 'statics', 'statics-docs'],
...
function preRender(subDir) {
  return through.obj(function(file, enc, cb) {
    ...
    var components = [];
    src = src.replace(/<!--\s*React\(\s*(.*)\s*\)\s*-->/g, function(_, relComponent) {
